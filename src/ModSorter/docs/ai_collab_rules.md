...
- deployer ※実機確認済み …
  動力軸は facing ではなく properties["axis_along_first"] で決まる(saw と同じ考え方)。
  ・axis_along_first=true → 垂直軸(y)。上面/下面の両端2面のいずれか1面に shaft(axis=y)。
  ・axis_along_first=false → facing に垂直な水平軸。
      facing=east/west(X向き) → 南北(z)の north/south。
      facing=north/south(Z向き) → 東西(x)の east/west。
  その軸の両端2面のいずれか1面に同軸 shaft があれば合格(片側でOK)。動力軸に沿わない面・
  前面(作用面)には繋がらない。作用は facing の向きの2マス先(1マス先は貫通)。
  実機教訓: facing=west/axis_along_first=true のとき south に axis=z の shaft を置いても
  噛み合わなかった(=facing基準の判定は偽合格を生む)。axis_along_first を軸の一次情報とする。
  → ConnectionValidator (A'''') の IsDeployer 判定で扱う。AutoFix はせず、両端2面とも
  未接続なら再生成 Issue(saw の A''' と同じ挙動)。
