...
- crushing_wheel（実機ID単数。複数形 crushing_wheels は誤りで検証が発火しなかった）…
  axis が回転軸。確定形は「横並び・真上投入・真下排出」。2個1組で、axis に垂直な水平方向へ
  1ブロック離して並べる（間に1マスの隙間。密着も2マス以上もダメ）。両輪を逆回転で駆動し、
  動力は各 wheel の axis 端（軸方向の隣）に shaft を同軸で挿す（垂直な側面は繋がらない）。
  素材は隙間の真上から投入。加工物は隙間の真下の保管庫（chest/barrel/item_vault）で受ける。
  受けに depot は不可（1個しか貯められず連続排出で詰まる）。本体側面に funnel を付ける
  millstone 方式でもない。
  → RequiresFunnelOutput には含めず、ConnectionValidator (C-0) の専用検証で扱う。
    (C-0) は (1)ペア存在 (2)1マス間隔 (3)両輪に軸端動力 (4)隙間真下の保管庫 を
    AutoFixable=false の再生成 Issue として検出する（自動補正はしない）。
