...
機走の12船種が共通で使うので `HullExpander.Screw.cs` の1枚に置く。増やした
プロパティは `hull_screws`（軸数0〜2）だけで、径・軸の勾配・舵の前後長は
主要目（喫水・船尾の立ち上がり・船尾の幅）から出す。寸法を個別に持たせると
「径が喫水より大きい」「舵が船体から離れて浮く」状態をUIから作れてしまう。
...
- 羽根は実物3〜5枚で振動を避けるため奇数が好まれるが、格子では90度おきの
  4枚しか置けない。
- 中心線舵（`hull_stern_rudder`）と z=-1 を奪い合うので併用しない。
  中心線舵を選んだときは推進器側の舵を降りる。
