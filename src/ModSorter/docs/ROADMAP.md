...
| 中分類 | 小分類 | 実装済み | 節ファイル |
|---|---:|---:|---|
| 小型艇 (`small_craft`) | 5 | 1 | [hull-small_craft.md](roadmap/hull-small_craft.md) |
...
### フェーズ6: 船種の網羅実装（31種） ── 進行中 17/31
残り: 小型艇4・作業船2・近代軍艦5・特殊・空想3。
小型艇で開放艇モード（`hull_open_boat` / `hull_thwart_step`）を新設した。
次の対象は小型艇の残り（モーターボート・スピードボート・ヨット・双胴船）。
