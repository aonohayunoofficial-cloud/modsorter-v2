...
public sealed partial class HullParamsControl
{
    ...
    {
        _ui.Heading("開放艇・漕ぎ座")
           .Toggle("open_boat", "甲板を張らない（開放艇）", "甲板を張る", p.OpenBoat)
           .Note("端艇（32ftカッター・27ftホエラー）は甲板を持たない。" +
                 "最上段のうち舷側の1列だけを甲板材で塗って舷縁（ガンネル）とし、" +
                 "内側は空けて船底へ床板を張る。船首材の頂と船尾の上端は閉じる。")
           .IntSlider("thwart", "漕ぎ座の間隔", 0, 32, p.ThwartStep,
               "0でなし。1は2へ丸める。甲板を張る設定では座は置かない")
           .Note("漕ぎ座は舷縁の高さで左右の舷をつなぐ板。実艇の座の間隔は0.9〜1.1mだが、" +
                 "1マス=1mで隣接させると座が連なって甲板と見分けが付かないので" +
                 "フレームと同じく2マス以上へ丸める。素材は舵と同じものを使う。" +
                 "床板は船底外板の1段上へ張り、素材はフレームと同じものを使う。");
        ...
        _ui.Heading("舵・推進器")
           ...
           .IntSlider("screws", "スクリューの軸数", 0, 2, p.Screws,
               "0でなし。バートラム46は2基2軸。リバティ船・タグボートは1軸")
           .Note("プロペラの径は喫水の0.65倍を上限に、船尾で船底が上がってできる" +
                 "空き（アパーチャ）＋1マスへ収める。軸は4マスで1マス上がる勾配で" +
                 "船底まで走らせ、そこが船内への貫通口になる。実艇の軸傾斜10〜13度に" +
                 "当たる。素材は舵と同じものを使う。" +
                 "軸を入れるとプロペラの後ろへ舵が立つので、中心線舵とは併用しない" +
                 "（同じ位置を使うため、中心線舵を選んだときは推進器側の舵を置かない）。");
        ...
        _ui.Heading("盾掛け・櫂・飾り")
           ...
           .IntSlider("row_oars", "櫂の数（片舷）", 0, 32, p.RowOars,
               "ガレーは漕ぎ座 片舷24。32ftカッターは片舷6で計12挺")
           .Note("櫂は舷縁から1マスごとに1段下げて水面へ向け、水面の手前で止まる。" +
                 "素材はマストと同じものを使う。外へ出るのは最大3マスだが、" +
                 "水面の手前で止まるので乾舷が1マスの端艇では1マスしか出ない。" +
                 "外寸の幅はこの実際の張り出しから取るので、櫂を付けても" +
                 "乾舷ぶんしか広がらない。")
